...
func add_shop_modifier(shop: Shop) -> void:
	...
	assert(shop_cost_modifier, "No shop cost modifier found")
	
	var coupons_modifier_value := shop_cost_modifier.get_value("coupons")
	
	if not coupons_modifier_value:
		coupons_modifier_value = ModifierValue.create_new_modifier("couponts", ModifierValue.Type.PERCENT_BASED)
		coupons_modifier_value.percent_value = -1 * discount / 100.0
		shop_cost_modifier.add_value(coupons_modifier_value)
